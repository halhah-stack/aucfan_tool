...
**目的**: 機能追加がやりやすい土台を作る。バグが減り、修正箇所がわかりやすくなる。
...
| タスク | 内容 | 状態 |
|---|---|---|
| 1688調査タブ（メインアプリ統合） | 商品カードに「🔍 1688」ボタン追加済み。1688トップページを開くのみ（画像検索自動化は断念） | ✅ 完了（2026-05-30・暫定） |
...
- [x] `GDRIVE_EMAIL` を `.env` に移行（2026-05-29）
- [x] Excel保存先をリサーチシート/ に分離（2026-05-29）
- [x] Excelファイル削除ボタン追加（2026-05-29）
- [x] FBAシミュレータ新タブ対応（2026-05-29）
...
## 1688画像検索自動化について（2026-05-30 調査・断念）

### 経緯

商品カードのAliPriceボタンの代替として、AucFan商品画像で1688画像検索を自動実行するボタンを実装しようとした。

### 試みた方法と結果

| 方法 | 結果 | 理由 |
|---|---|---|
| `imageUrl`パラメーターをURLに付与 | 失敗 | 1688がパラメーターを無視（テキスト検索に落ちる） |
| Seleniumで`input#img-search-upload`にsend_keys | 失敗 | 1688のJSがTrusted Eventを検知できずに反応しない |
| change/inputイベントをdispatch | 失敗 | 同上 |
| Cmd+V（pyperclip + ActionChains） | 失敗 | 1688ページのbodyへの貼り付けは無効 |
| JS ClipboardEvent('paste')をdispatch | 失敗 | 同上 |
| `imageAddress`パラメーターでURL直接指定 | 失敗 | Bot検知 → スライドキャプチャが表示される |
| ユーザーが手動Cmd+Vでペースト | 画像表示→即リバート | Yahoo CDNのホットリンクブロックにより1688サーバーが画像を取得できない |

### 結論

**1688のセキュリティが堅牢なため自動化は断念。**

- Trusted Event検知（本物のユーザー操作か機械操作かを判定）
- ホットリンクブロック（外部URLからの画像取得を拒否）
- Bot検知 + スライドキャプチャ（異常なアクセスパターンを検出）

### 現状の実装

- 商品カードに「🔍 1688」ボタン（`static/app.js`）を追加
- クリックで1688トップページ（`https://s.1688.com/selloffer/offer_search.htm`）を新タブで開く
- 画像検索はユーザーが手動でカメラアイコンからファイル選択
- **AliPriceボタンも引き続き残存**（使い勝手が良いため）

### 将来的な改善案

- `pyautogui`で物理的なマウス操作（要座標調整・不安定）
- ローカルサーバー画像をBase64 ClipboardItemでペースト（ブラウザ対応限定）
- SP-API承認後はAmazon側の自動化を優先する
