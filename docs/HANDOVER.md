...
~/マイドライブ/AucFanToolData/リサーチ結果/
  商品名/
    ...
    1688/                          ← 1688仕入れ画像（scraper_1688.py が保存）
```
...
```
1. 実動テスト（1688スクレイパー）
   └── ./start.sh 後に /research で1688 URLを貼り付けて動作確認
   └── ショップ名が取れない場合は check_1688_detail.py でショップリンクのhrefを確認

2. Claude Excel アドイン連携（ユーザー要望・詳細未定）
```
...
- バリアント：body テキストのバリアントセクション（`规格`/`尺寸`/`颜色`/`型号`/`款式`/`套餐` に対応）
  - フォーマットA: 名称 / ¥価格 / 库存N（3行別々）
  - フォーマットB: 名称 / ¥価格库存N（価格と在庫が同一行）← 両対応
- ショップ：`shop*.1688.com` サブドメイン形式で名前・URL取得、bodyテキストから評価・回頭率・入驻年数
- 画像：`img[src*='cbu01']`

**戻り値に `shop_years`（例: "9年"/"3ヶ月"）を追加**
...
- ショップ名がまだ取れない場合は `check_1688_detail.py` でショップリンクの実際のhrefを確認すること
...
| G | 商品名（日本語） | 手入力（薄黄背景） |
| H | バリアント（中国語） | 自動 |
| I | バリアント（日本語） | 手入力（薄黄背景） |
| J | 在庫数 | 自動 |
| K | 単価(CNY) | 自動 |
| L | MOQ | 自動 |
| M | 仕入総額(CNY) = K×L | 数式 |
| N | 仕入総額(JPY) = K×L×係数 | 数式 |
| O | 原価/個(JPY) = K×係数 | 数式 |
| P | 利益(JPY) = 販売価格-FBA-O | 数式 |
| Q | 利益率 | 数式 |
| R | 判定（◎ GO / × 再検討） | 数式 |

- 係数（35）は `.env` の `CNY_TO_JPY_RATE=35` で変更可能
- 販売価格 = Sheet1!B12、FBA手数料 = Sheet1!B13
- **既存Excelは列が古い形式のため、新規作成すると新レイアウトが適用される**
