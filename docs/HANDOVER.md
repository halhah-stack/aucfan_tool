...
| 優先 | タスク | リスク | 効果 |
|---|---|---|---|
| 高 | `app.py` の `/research` HTMLを `templates/research.html` に切り出し | 小 | JSバグが減る・デバッグしやすくなる |
| 高 | `_parse_item_card()` の除外判定を別メソッドに切り出し | 小 | 除外ルール追加が楽になる |
| 中 | `app.py` のAPIルートを `routes/` フォルダに分割 | 中 | 3900行のファイルが管理しやすくなる |
| 中 | `app.py` のビジネスロジックを `services/` に分離 | 中 | テストが書きやすくなる |
| 低 | グローバル状態（`_seller_state` 等）をクラスに封じ込める | 中 | 並列処理に備えた設計 |
