...
# ─────────────────────────────────────────────
# エクスポート（services/export.py に移行済み）
# ─────────────────────────────────────────────

from services.export import _generate_export_html, _save_export_files, _gdrive_copy_html, _build_export_html
...
# セッション管理ロジックは services/session.py に移行済み
from services.session import parse_session_info, list_sessions as _list_sessions_impl


def _parse_session_info(name: str) -> dict:
    """後方互換ラッパー → services.session.parse_session_info に委譲"""
    return parse_session_info(name)
...
def _list_sessions(step: int = None):
    ...
    if not base.exists() and str(base).startswith(_gdrive_prefix):
        base = Path("リサーチ結果")
    return _list_sessions_impl(base, running_names, step=step)
...
# ─────────────────────────────────────────────
# /research ルートは routes/research.py (Blueprint) に移行済み
