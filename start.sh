...
# .env からFlaskポートを読み込む（デフォルト5001）
FLASK_PORT=$(grep -E '^FLASK_PORT=' .env 2>/dev/null | cut -d= -f2 | tr -d ' ')
...
echo ""
echo "次の手順:"
echo "  1. AucFanにログイン・検索条件を設定・1ページ目を表示"
echo "  2. http://localhost:${FLASK_PORT} のタブに戻ってスクレイピング開始"
